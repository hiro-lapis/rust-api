...
use kernel::model::{book::event::UpdateBook, checkout::{event::{CreateCheckout, UpdateReturned}, Checkout}, id::{BookId, CheckoutId}};
...
pub async fn checkout_book(
    ...
    registry.checkout_repository().create(create_checkout_history).await.map(|_| StatusCode::CREATED)
}
...
pub async fn return_book(
    ...
    registry.checkout_repository().update_returned(update_returned).await.map(|_| StatusCode::OK)
}
...
pub async fn show_checked_out_list(
    ...
    registry.checkout_repository().find_unreturned_all().await.map(CheckoutsResponse::from).map(Json)
}
